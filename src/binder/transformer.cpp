...
bool Parser::TransformParseTree(duckdb_libpgquery::PGList *tree, vector<unique_ptr<SQLStatement>> &statements) {
...
unique_ptr<SQLStatement> Parser::TransformStatement(duckdb_libpgquery::PGNode *stmt) {
  switch (stmt->type) {
...
TypeId Parser::TransformTypeId(duckdb_libpgquery::PGTypeName *type_name) {
...
Column Parser::TransformColumnDefinition(duckdb_libpgquery::PGColumnDef *cdef) {
  string colname;
...
Value Parser::TransformConstant(duckdb_libpgquery::PGAConst *c) {
...
void Parser::TransformExpressionList(duckdb_libpgquery::PGList &list, vector<Value> &result) {
