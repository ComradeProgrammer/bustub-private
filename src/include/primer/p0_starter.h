...
class RowMatrixOperations {
 public:
  ...
  static auto Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) -> std::unique_ptr<RowMatrix<T>> {
    ...
    if (matrixB->GetColumnCount() != col || matrixB->GetRowCount() != row) {
      return std::unique_ptr<RowMatrix<T>>(nullptr);
    }
    auto *res = new RowMatrix<T>(row, col);
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < col; j++) {
  ...
  static auto Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) -> std::unique_ptr<RowMatrix<T>> {
    ...
    int intermediate = matrixA->GetColumnCount();
    if (matrixB->GetRowCount() != intermediate) {
      return std::unique_ptr<RowMatrix<T>>(nullptr);
    }
    auto *res = new RowMatrix<T>(row, col);
    for (int i = 0; i < row; i++) {
  ...
  static auto GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB, const RowMatrix<T> *matrixC)
    ...
    // TODO(P0): Add implementation
    if (matrixB->GetRowCount() != matrixA->GetColumnCount()) {
      return std::unique_ptr<RowMatrix<T>>(nullptr);
    }
    ...
    std::unique_ptr<RowMatrix<T>> res = Add(tmp.get(), matrixC);
    return res;
  }
};
}  // namespace bustub
